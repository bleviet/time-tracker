...
class TimerService(QObject):
    ...
    def _schedule_background_save(self) -> None:
        """Schedule a background save, falling back to a blocking run if needed."""
        try:
        ...
        if loop and loop.is_running():
            loop.create_task(self._background_save())
        else:
            asyncio.get_event_loop().run_until_complete(self._background_save())
